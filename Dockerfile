...
# 2. Run docker container: docker run -d --name gmbd-backend-spring-app -p 8080:8080 gmbd-backend-spring-image (create, start, and map port 8080 of the container to port 8080 of the host)
...
# docker kill gmbd-backend-spring-app, to kill running docker process (stop it immediately)
